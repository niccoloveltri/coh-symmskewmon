...
ex⊗l⋆ : ∀ S {Γ} Λ {Δ A B C} b
  → (f : S ∣ Λ ++ Γ ++ A ∷ B ∷ Δ ⊢ C)
  → ex b (⊗l⋆ S Λ {Γ ++ A ∷ B ∷ Δ} f) ≗ ⊗l⋆ S Λ (ex {Γ = Λ ++ Γ} b f)
ex⊗l⋆ nothing [] b f = exIl-b b
ex⊗l⋆ nothing {Γ} (x ∷ Λ) b f =
  ex⊗l⋆ _ Λ b (⊗l (Il f))
  ∙ cong⊗l⋆ {just (I ⊗ x)} {Λ} (ex⊗l-b {Γ = Λ ++ Γ} b ∙ ⊗l (exIl-b b))
ex⊗l⋆ (just x) [] b f = refl
ex⊗l⋆ (just x) {Γ} (x₁ ∷ Λ) b f =
  ex⊗l⋆ _ Λ b (⊗l f) ∙ cong⊗l⋆ {just (x ⊗ x₁)} {Λ} (ex⊗l-b {Γ = Λ ++ Γ} b)
...
cmplt-⊗r-ψ {A}{B}{C ∷ Δ}  =
  ...
  ≗〈 cong⊗l⋆ {Γ = Δ} (~ (scutscut-vass (cmplt α) (⊗l-1⋆ (just (A ⊗ (B ⊗ C))) Δ {[]} ax) _)) 〉 
...
cmplt-⊗r-lemma {just A} {[]} {Δ}  =
  ...
  ≗〈 cong⊗l⋆ {Γ = Δ} (~ (scutscut-vass (⊗r ax Ir) (⊗l-1⋆ (just (A ⊗ I)) Δ ax) _)) 〉
...
cmplt-⊗r-lemma {nothing} {[]} {Δ} =
  ...
  ≗〈 cong⊗l⋆ {Γ = Δ} (~ (scutscut-vass (⊗r ax Ir) (⊗l-1⋆ (just (I ⊗ I)) Δ ax) _)) 〉
...
cong-strcmplt {S}{Γ} p = cong⊗l-1⋆ {S} {Γ} (eq-cmplt p)
