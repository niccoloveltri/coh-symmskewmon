...
scut-ex-fma-cxt : ∀{S Γ Δ₁ Δ₂} Λ {A' A C b}
  → (f : S ∣ Δ₁ ++ A ∷ Λ ++ Δ₂ ⊢ A') (g : just A' ∣ Γ ⊢ C)
  → scut (ex-fma-cxt {Γ = Δ₁}{Δ₂} Λ b f) g ≡ ex-fma-cxt {Γ = Δ₁} Λ b (scut f g)
scut-ex-fma-cxt [] f g = refl
scut-ex-fma-cxt {Δ₁ = Δ₁} (x ∷ Λ) f g =
  scut-ex-fma-cxt {Δ₁ = Δ₁ ++ x ∷ []} Λ (ex _ f) g
  
scut-ex-cxt-fma : ∀{S Γ Δ₁ Δ₂} Λ {A' A C b}
  → (f : S ∣ Δ₁ ++ Λ ++ A ∷ Δ₂ ⊢ A') (g : just A' ∣ Γ ⊢ C)
  → scut (ex-cxt-fma {Γ = Δ₁}{Δ₂} Λ b f) g ≡ ex-cxt-fma {Γ = Δ₁} Λ b (scut f g)
scut-ex-cxt-fma [] f g = refl
scut-ex-cxt-fma {Δ₁ = Δ₁} (x ∷ Λ) f g =
  cong (ex _) (scut-ex-cxt-fma {Δ₁ = Δ₁ ++ x ∷ []} Λ f g)

scut-ex : ∀{S Γ Δ₁ Δ₂ A' A B C} b
  → (f : S ∣ Γ ⊢ A') {g : just A' ∣ Δ₁ ++ A ∷ B ∷ Δ₂ ⊢ C}
  → scut f (ex b g) ≗ ex {Γ = Γ ++ Δ₁} b (scut f g)
scut-ex b ax = refl
scut-ex {Δ₁ = Δ} b (uf {Γ} f) =
  uf (scut-ex b f) ∙ ~ exuf-b {Γ = Γ ++ Δ} b
scut-ex {Δ₁ = Δ} b (ex {Δ = Γ} b' f) =
  ex b' (scut-ex b f) ∙ ~ exex-b {Γ₂ = Γ ++ Δ} b
scut-ex b Ir = refl
scut-ex b (⊗r f f') = refl
scut-ex {Γ = Γ} {Δ} b (Il f) = 
  Il (scut-ex b f) ∙ ~ exIl-b {Γ = Γ ++ Δ} b
scut-ex {Γ = Γ} {Δ} b (⊗l f) =
  ⊗l (scut-ex b f) ∙ ~ ex⊗l-b {Γ = Γ ++ Δ} b

scut-ex-fma-cxt2 : ∀{S Γ Δ₁ Δ₂} Λ {A' A C b}
  → (f : S ∣ Γ ⊢ A') (g : just A' ∣ Δ₁ ++ A ∷ Λ ++ Δ₂ ⊢ C)
  → scut f (ex-fma-cxt {Γ = Δ₁}{Δ₂} Λ b g) ≗ ex-fma-cxt {Γ = Γ ++ Δ₁} Λ b (scut f g)
scut-ex-fma-cxt2 [] f g = refl
scut-ex-fma-cxt2 {Γ = Γ} {Δ₁}{Δ₂} (x ∷ Λ) f g =
  scut-ex-fma-cxt2 {Δ₁ = Δ₁ ++ x ∷ []}{Δ₂} Λ f (ex _ g)
  ∙ cong-ex-fma-cxt Λ _ (scut-ex _ f)

scut-ex-cxt-fma2 : ∀{S Γ Δ₁ Δ₂} Λ {A' A C b}
  → (f : S ∣ Γ ⊢ A') (g : just A' ∣ Δ₁ ++ Λ ++ A ∷ Δ₂ ⊢ C)
  → scut f (ex-cxt-fma {Γ = Δ₁} Λ b g) ≗ ex-cxt-fma {Γ = Γ ++ Δ₁} Λ b (scut f g)
scut-ex-cxt-fma2 [] f g = refl
scut-ex-cxt-fma2 {Γ = Γ}{Δ₁} (x ∷ Λ) f g =
  scut-ex _ f ∙ ex {Γ = Γ ++ Δ₁} _ (scut-ex-cxt-fma2 {Δ₁ = Δ₁ ++ _ ∷ []} Λ f g)

scut⊗r-ex-fma-cxt : ∀{S Γ₁ Γ₂ Δ₁ Δ₂} Λ {A' B' A C b}
  → (f : S ∣ Γ₁ ⊢ A') (f' : nothing ∣ Γ₂ ⊢ B') (g : just (A' ⊗ B') ∣ Δ₁ ++ A ∷ Λ ++ Δ₂ ⊢ C)
  → scut (⊗r f f') (ex-fma-cxt {Γ = Δ₁}{Δ₂} Λ b g) ≡ ex-fma-cxt {Γ = Γ₁ ++ Γ₂ ++ Δ₁} Λ b (scut (⊗r f f') g)
scut⊗r-ex-fma-cxt [] f f' g = refl
scut⊗r-ex-fma-cxt {Δ₁ = Δ₁} (x ∷ Λ) f f' g =
  scut⊗r-ex-fma-cxt {Δ₁ = Δ₁ ++ x ∷ []} Λ f f' (ex _ g)

scut⊗r-ex-cxt-fma : ∀{S Γ₁ Γ₂ Δ₁ Δ₂} Λ {A' B' A C b}
  → (f : S ∣ Γ₁ ⊢ A') (f' : nothing ∣ Γ₂ ⊢ B') (g : just (A' ⊗ B') ∣ Δ₁ ++ Λ ++ A ∷ Δ₂ ⊢ C)
  → scut (⊗r f f') (ex-cxt-fma {Γ = Δ₁}{Δ₂} Λ b g) ≡ ex-cxt-fma {Γ = Γ₁ ++ Γ₂ ++ Δ₁} Λ b (scut (⊗r f f') g)
scut⊗r-ex-cxt-fma [] f f' g = refl
scut⊗r-ex-cxt-fma {Γ₁ = Γ₁}{Γ₂}{Δ₁} (x ∷ Λ) f f' g =
  cong (ex {Γ = Γ₁ ++ Γ₂ ++ Δ₁} _) (scut⊗r-ex-cxt-fma {Δ₁ = Δ₁ ++ x ∷ []} Λ f f' g)

ccut-ex : ∀{T Γ₁ Γ₂ Δ} Δ₀ {Δ' A B A' C} (b : Bool)
  → (f : nothing ∣ Γ₁ ++ A ∷ B ∷ Γ₂ ⊢ A') (g : T ∣ Δ ⊢ C)
  → (eq : Δ ≡ Δ₀ ++ A' ∷ Δ')
  → ccut Δ₀ (ex b f) g eq ≗ ex {Γ = Δ₀ ++ Γ₁} b (ccut Δ₀ f g eq)
ccut-ex Δ₀ b f ax eq = ⊥-elim ([]disj∷ Δ₀ eq)
ccut-ex Δ₀ b f (uf g) eq with cases∷ Δ₀ eq
... | inj₁ (refl , refl , refl) = refl
ccut-ex {Γ₁ = Γ₁} _ b f (uf g) eq | inj₂ (Γ₀ , refl , refl) =
  uf (ccut-ex Γ₀ b f g refl) ∙ ~ (exuf-b {Γ₀ ++ Γ₁} b)
ccut-ex {Γ₁ = Γ₁}{Γ₂} Δ₀ {Δ'} b f (ex {Γ = Γ} {Δ} b' g) eq with cases++2 Δ₀ Γ Δ' Δ eq
... | inj₁ (Γ₀ , refl , refl) =
  ex {Γ = Δ₀ ++ Γ₁ ++ _ ∷ _ ∷ Γ₂ ++ Γ₀} b' (ccut-ex Δ₀ b f g refl)
  ∙ exex-b {Γ₁ = Δ₀ ++ Γ₁}{Γ₂ ++ Γ₀} _ 
... | inj₂ (inj₁ (Γ₀ , refl , refl)) =
  ex b' (ccut-ex (Γ ++ _ ∷ _ ∷ Γ₀) b f g refl) ∙ ~ exex-b {Γ₁ = Γ}{Γ₀ ++ Γ₁} _
... | inj₂ (inj₂ (inj₁ (refl , refl , refl))) =
  cong-ex-fma-cxt (Γ₁ ++ _ ∷ _ ∷ Γ₂) b' (ccut-ex (Δ₀ ++ _ ∷ []) b f g refl)
  ∙ ex-fma-cxt-ex-braid Γ₁ _
... | inj₂ (inj₂ (inj₂ (refl , refl , refl))) =
  cong-ex-cxt-fma (Γ₁ ++ _ ∷ _ ∷ Γ₂) b' (ccut-ex Γ b f g refl)
  ∙ ex-cxt-fma-ex-braid Γ₁ _ 
ccut-ex Δ₀ b f Ir eq = ⊥-elim ([]disj∷ Δ₀ eq)
ccut-ex {Γ₁ = Γ₁} Δ₀ {Δ'} b f (⊗r {Γ = Γ} {Δ} g g₁) eq with cases++ Δ₀ Γ Δ' Δ eq
... | inj₁ (Γ₀ , refl , refl) =
  ⊗r (ccut-ex Δ₀ b f g refl) refl ∙ ~ ex⊗r₁-b {Γ₁ = Δ₀ ++ Γ₁} b
... | inj₂ (Γ₀ , refl , refl) =
  ⊗r refl (ccut-ex Γ₀ b f g₁ refl) ∙ ~ ex⊗r₂-b {Δ₁ = Γ₀ ++ Γ₁} b
ccut-ex {Γ₁ = Γ₁} Δ₀ b f (Il g) eq =
  Il (ccut-ex Δ₀ b f g eq) ∙ ~ exIl-b {Γ = Δ₀ ++ Γ₁} b
ccut-ex {Γ₁ = Γ₁} Δ₀ b f (⊗l g) refl = 
  ⊗l (ccut-ex (_ ∷ Δ₀) b f g refl) ∙ ~ ex⊗l-b {Γ = Δ₀ ++ Γ₁} b

ccut-ex-fma-cxt1 : ∀{T Γ₁ Γ₂ Δ} Δ₀ Λ {Δ' A A' C b}
  → (f : nothing ∣ Γ₁ ++ A ∷ Λ ++ Γ₂ ⊢ A') (g : T ∣ Δ ⊢ C)
  → (eq : Δ ≡ Δ₀ ++ A' ∷ Δ')
  → ccut Δ₀ (ex-fma-cxt Λ b f) g eq ≗ ex-fma-cxt {Γ = Δ₀ ++ Γ₁}{Γ₂ ++ Δ'} Λ b (ccut Δ₀ f g eq)
ccut-ex-fma-cxt1 Δ₀ [] f g eq = refl
ccut-ex-fma-cxt1 {Γ₁ = Γ₁} {Γ₂} Δ₀ (x ∷ Λ) {Δ'} f g refl =
  ccut-ex-fma-cxt1 {Γ₁ = Γ₁ ++ x ∷ []} Δ₀ Λ (ex _ f) g refl
  ∙ cong-ex-fma-cxt Λ _ (ccut-ex Δ₀ _ f g refl)

ccut-ex-fma-cxt2 : ∀{T Γ} Δ₀ Λ {Δ₁ Δ₂ A A' C b}
  → (f : nothing ∣ Γ ⊢ A') (g : T ∣ Δ₀ ++ A' ∷ Δ₁ ++ A ∷ Λ ++ Δ₂ ⊢ C)
  → ccut Δ₀ f (ex-fma-cxt {Γ = Δ₀ ++ A' ∷ Δ₁}{Δ₂} Λ b g) refl
    ≡ ex-fma-cxt {Γ = Δ₀ ++ Γ ++ Δ₁} Λ b (ccut Δ₀ f g refl)
ccut-ex-fma-cxt2 Δ₀ [] f g = refl
ccut-ex-fma-cxt2 Δ₀ (x ∷ Λ) {Δ₁}{Δ₂} f g with ccut-ex-fma-cxt2 Δ₀ Λ {Δ₁ ++ x ∷ []}{Δ₂} f (ex {Γ = Δ₀ ++ _ ∷ Δ₁}{Λ ++ Δ₂} _ g)
ccut-ex-fma-cxt2 Δ₀ (x ∷ Λ) {Δ₁} {Δ₂} {A}{A'} f g | ih
  rewrite cases++-inj₁ Δ₀ Δ₁ (x ∷ A ∷ Λ ++ Δ₂) A' = ih

ccut-ex-fma-cxt2' : ∀{T Γ} Δ₀ Λ {Δ₁ Δ₂ A A' C b}
  → (f : nothing ∣ Γ ⊢ A') (g : T ∣ Δ₀ ++ A ∷ Λ ++ Δ₁ ++ A' ∷ Δ₂ ⊢ C)
  → ccut (Δ₀ ++ Λ ++ A ∷ Δ₁) f (ex-fma-cxt {Γ = Δ₀}{Δ₁ ++ A' ∷ Δ₂} Λ b g) refl
    ≡ ex-fma-cxt {Γ = Δ₀} Λ b (ccut (Δ₀ ++ A ∷ Λ ++ Δ₁) f g refl)
ccut-ex-fma-cxt2' Δ₀ [] f g = refl
ccut-ex-fma-cxt2' {Γ = Γ} Δ₀ (x ∷ Λ) {Δ₁} {Δ₂} {A}{A'} f g with ccut-ex-fma-cxt2' {Γ = Γ} (Δ₀ ++ x ∷ []) Λ {Δ₁}{Δ₂} f (ex _ g)
... | ih rewrite cases++-inj₂ (x ∷ A ∷ Λ ++ Δ₁) Δ₀ Δ₂ A' = ih

ccut-ex-cxt-fma : ∀{T Γ₁ Γ₂ Δ} Δ₀ Λ {Δ' A A' C b}
  → (f : nothing ∣ Γ₁ ++ Λ ++ A ∷ Γ₂ ⊢ A') (g : T ∣ Δ ⊢ C)
  → (eq : Δ ≡ Δ₀ ++ A' ∷ Δ')
  → ccut Δ₀ (ex-cxt-fma {Γ = Γ₁} Λ b f) g eq ≗ ex-cxt-fma {Γ = Δ₀ ++ Γ₁} Λ b (ccut Δ₀ f g eq)
ccut-ex-cxt-fma Δ₀ [] f g eq = refl
ccut-ex-cxt-fma {Γ₁ = Γ₁} {Γ₂} Δ₀ (x ∷ Λ) f g refl =
  ccut-ex Δ₀ _ (ex-cxt-fma {Γ = Γ₁ ++ _ ∷ []} Λ _ f) g refl
  ∙ ex {Γ = Δ₀ ++ Γ₁} _ (ccut-ex-cxt-fma {Γ₁ = Γ₁ ++ x ∷ []} Δ₀ Λ f g refl)

ccut-ex-cxt-fma2 : ∀{T Γ} Δ₀ Λ {Δ₁ Δ₂ A A' C b}
  → (f : nothing ∣ Γ ⊢ A') (g : T ∣ Δ₀ ++ A' ∷ Δ₁ ++ Λ ++ A ∷ Δ₂ ⊢ C)
  → ccut Δ₀ f (ex-cxt-fma {Γ = Δ₀ ++ A' ∷ Δ₁}{Δ₂} Λ b g) refl
    ≡ ex-cxt-fma {Γ = Δ₀ ++ Γ ++ Δ₁} Λ b (ccut Δ₀ f g refl)
ccut-ex-cxt-fma2 Δ₀ [] f g = refl
ccut-ex-cxt-fma2 {Γ = Γ} Δ₀ (x ∷ Λ) {Δ₁} {Δ₂} {A}{A'} f g
  rewrite cases++-inj₁ Δ₀ Δ₁ (A ∷ x ∷ Λ ++ Δ₂) A' =
  cong (ex {Γ = Δ₀ ++ Γ ++ Δ₁} _) (ccut-ex-cxt-fma2 Δ₀ Λ {Δ₁ ++ x ∷ []} f g )

ccut-ex-cxt-fma2' : ∀{T Γ} Δ₀ Λ {Δ₁ Δ₂ A A' C b}
  → (f : nothing ∣ Γ ⊢ A') (g : T ∣ Δ₀ ++ Λ ++ A ∷ Δ₁ ++ A' ∷ Δ₂ ⊢ C)
  → ccut (Δ₀ ++ A ∷ Λ ++ Δ₁) f (ex-cxt-fma {Γ = Δ₀}{Δ₁ ++ A' ∷ Δ₂} Λ b g) refl
    ≡ ex-cxt-fma {Γ = Δ₀}{Δ₁ ++ Γ ++ Δ₂} Λ b (ccut (Δ₀ ++ Λ ++ A ∷ Δ₁) f g refl)
ccut-ex-cxt-fma2' Δ₀ [] f g = refl
ccut-ex-cxt-fma2' {Γ = Γ} Δ₀ (x ∷ Λ) {Δ₁} {Δ₂} {A} {A'} f g
  rewrite cases++-inj₂ (A ∷ x ∷ Λ ++ Δ₁) Δ₀ Δ₂ A' =
    cong (ex _) (ccut-ex-cxt-fma2' {Γ = Γ} (Δ₀ ++ x ∷ []) Λ {Δ₁} {Δ₂} f g)

ccut-ex-cxt-cxt2 : ∀{T Γ} Δ₀ Λ {Δ₁ A C b}
  → (f : nothing ∣ Γ ⊢ A) (g : T ∣ Δ₀ ++ Λ ++ A ∷ Δ₁ ⊢ C)
  → ccut Δ₀ f (ex-cxt-fma {Γ = Δ₀}{Δ₁} Λ b g) refl
    ≡ ex-cxt-cxt1 {Γ = Δ₀} {Δ₁} Λ Γ b (ccut (Δ₀ ++ Λ) f g refl) 
ccut-ex-cxt-cxt2 Δ₀ [] f g = refl
ccut-ex-cxt-cxt2 {Γ = Γ} Δ₀ (x ∷ Λ) {Δ₁} {A} f g
  rewrite cases++-inj₂ [] Δ₀ (x ∷ Λ ++ Δ₁) A =
  cong (ex-fma-cxt Γ _) (ccut-ex-cxt-cxt2 {Γ = Γ} (Δ₀ ++ x ∷ []) Λ {Δ₁} f g)

ccut-ex-cxt-cxt2' : ∀{T Γ} Δ₀ Λ {Δ₁ A C b}
  → (f : nothing ∣ Γ ⊢ A) (g : T ∣ Δ₀ ++ A ∷ Λ ++ Δ₁ ⊢ C)
  → ccut (Δ₀ ++ Λ) f (ex-fma-cxt {Γ = Δ₀}{Δ₁} Λ b g) refl
    ≡ ex-cxt-cxt2 {Γ = Δ₀} {Δ₁} Γ Λ b (ccut Δ₀ f g refl) 
ccut-ex-cxt-cxt2' Δ₀ [] f g = refl
ccut-ex-cxt-cxt2' {Γ = Γ} Δ₀ (x ∷ Λ) {Δ₁} {A} f g with ccut-ex-cxt-cxt2' {Γ = Γ} (Δ₀ ++ x ∷ []) Λ {Δ₁} f (ex _ g)
...
ccut-unit {Γ = Γ} Δ₀ (ex {Γ = Γ₁} {Δ} b g) eq with cases++2 Δ₀ Γ₁ Γ Δ eq
ccut-unit Δ₀ (ex b g) refl | inj₁ (Γ₀ , refl , refl) =
  cong (ex {Γ = Δ₀ ++ _ ∷ Γ₀} b) (ccut-unit Δ₀ g refl)
ccut-unit _ (ex {Γ = Γ₁} b g) refl | inj₂ (inj₁ (Γ₀ , refl , refl)) =
  cong (ex b) (ccut-unit (Γ₁ ++ _ ∷ _ ∷ Γ₀) g refl)
ccut-unit Δ₀ (ex b g) refl | inj₂ (inj₂ (inj₁ (refl , refl , refl))) =
  cong (ex b) (ccut-unit (Δ₀ ++ _ ∷ []) g refl)
ccut-unit _ (ex {Γ = Γ} b g) refl | inj₂ (inj₂ (inj₂ (refl , refl , refl))) =
  cong (ex b) (ccut-unit Γ g refl)
...
scut-unit (ex b f) = cong (ex b) (scut-unit f)

Il-1-scutIr : {Γ : Cxt} → {C : Fma}
  → (f : just I ∣ Γ ⊢ C)
  → Il-1 f ≡ scut Ir f
Il-1-scutIr ax = refl
Il-1-scutIr (ex b f) = cong (ex b) (Il-1-scutIr f)
...
⊗l-1-scut⊗r (ex {Γ = Γ} b f) = cong (ex {Γ = _ ∷ Γ} b) (⊗l-1-scut⊗r f)
...
⊗l-1-alt (ex {Γ = Γ} b f) = cong (ex {Γ = _ ∷ Γ} b) (⊗l-1-alt f)
...
scutIl-1 (⊗r {Γ = Γ} {Δ} f f') (ex {Γ = Γ₁} b g) =
  cong (ex {Γ = Γ ++ Δ ++ Γ₁} b) (scutIl-1 (⊗r f f') g)
scutIl-1 (Il f) g = refl
scutIl-1 (ex b f) g = cong (ex b) (scutIl-1 f g)
...
ccutIl-1 Δ₀ {Δ'} f (ex {Γ = Γ} {Δ} b g) r with cases++2 Δ₀ Γ Δ' Δ r
ccutIl-1 {Γ} Δ₀ {.(Γ₀ ++ _ ∷ _ ∷ Δ)} f (ex {Γ = .(Δ₀ ++ _ ∷ Γ₀)} {Δ} b g) r | inj₁ (Γ₀ , refl , refl) =
  cong (ex {Γ = Δ₀ ++ Γ ++ Γ₀} b) (ccutIl-1 Δ₀ f g refl)
... | inj₂ (inj₁ (Γ₀ , refl , refl)) = cong (ex b) (ccutIl-1 (Γ ++ _ ∷ _ ∷ Γ₀) f g refl)
ccutIl-1 {Γ} Δ₀ {.(_ ∷ Δ)} f (ex {Γ = .Δ₀} {Δ} b g) r | inj₂ (inj₂ (inj₁ (refl , refl , refl))) =
  trans (sym (ex-fma-cxt-Il-1 Δ₀ Γ _))
        (cong (ex-fma-cxt Γ b) (ccutIl-1 (Δ₀ ++ _ ∷ []) f g refl))
ccutIl-1 {Γ₁} .(Γ ++ _ ∷ []) {Δ'} f (ex {Γ = Γ} {.Δ'} b g) r | inj₂ (inj₂ (inj₂ (refl , refl , refl))) =
  trans (sym (ex-cxt-fma-Il-1 Γ Γ₁ _))
        (cong (ex-cxt-fma Γ₁ b) (ccutIl-1 Γ f g refl))
...
scut⊗l-1 (⊗r {Γ = Γ} {Δ} f f') (ex {Γ = Γ'} b g) =
  cong (ex {Γ = _ ∷ Γ ++ Δ ++ Γ'} b) (scut⊗l-1 (⊗r f f') g)
scut⊗l-1 (⊗l f) g = refl
scut⊗l-1 (ex {Γ = Γ} b f) g = cong (ex {Γ = _ ∷ Γ} b) (scut⊗l-1 f g)
...
scut⊗ruf {f = f}{g} (⊗r h h') =
  ...
  qed≗
scut⊗ruf (⊗l h) = refl
scut⊗ruf {Γ} {Δ} {f = f} {g} (ex {Γ = Λ} b h) =
  proof≗
    ex b (scut (⊗r (uf f) g) h)
  ≗〈 ex b (scut⊗ruf h) 〉
    ex b (uf (scut (⊗r f g) h))
  ≗〈 exuf-b {Γ = Γ ++ Δ ++ Λ} b 〉
    uf (ex {Γ = Γ ++ Δ ++ Λ} b (scut (⊗r f g) h))
  qed≗
...
scut⊗rIl {f = f}{g}(⊗r h h') =
  ...
  qed≗
scut⊗rIl (⊗l h) = refl
scut⊗rIl {Γ} {Δ} {f = f} {g} (ex {Γ = Λ} b h) = 
  proof≗
    ex {Γ = Γ ++ Δ ++ Λ} b (scut (⊗r (Il f) g) h)
  ≗〈 ex {Γ = Γ ++ Δ ++ Λ} b (scut⊗rIl h) 〉
    ex {Γ = Γ ++ Δ ++ Λ} b (Il (scut (⊗r f g) h))
  ≗〈 exIl-b {Γ = Γ ++ Δ ++ Λ} b 〉
    Il (ex {Γ = Γ ++ Δ ++ Λ} b (scut (⊗r f g) h))
  qed≗
...
scut⊗r⊗l {f = f}{g} (⊗r h h') =
  ...
  qed≗
scut⊗r⊗l (⊗l h) = refl
scut⊗r⊗l {Γ} {Δ} {f = f} {g} (ex {Γ = Λ} b h) = 
  proof≗
    ex {Γ = Γ ++ Δ ++ Λ} b (scut (⊗r (⊗l f) g) h)
  ≗〈 ex {Γ = Γ ++ Δ ++ Λ} b (scut⊗r⊗l h) 〉
    ex {Γ = Γ ++ Δ ++ Λ}  b(⊗l (scut (⊗r f g) h))
  ≗〈 ex⊗l-b {Γ = Γ ++ Δ ++ Λ} b 〉
    ⊗l (ex {Γ = _ ∷ Γ ++ Δ ++ Λ} b (scut (⊗r f g) h))
  qed≗
...
scut⊗r-hass {Γ₁ = Γ} {Δ} Δ₀ {Δ'} {A₁} {A₂ = A₂} f₁ f₁' f₂ (ex {Γ = Γ₁} {Δ₁} {A} {B} b g) eq with cases++2 Δ₀ Γ₁ Δ' Δ₁ eq
scut⊗r-hass {Γ₁ = Γ} {Δ} {Γ₂ = Γ₂} Δ₀ {A₂ = A₂} f₁ f₁' f₂ (ex {Γ = _} {Δ₁} {A} {B} b g) refl | inj₁ (Γ₀ , refl , refl)
   rewrite cases++-inj₁ (Γ ++ Δ ++ Δ₀) Γ₀ (B ∷ A ∷ Δ₁) A₂ =
   ex {Γ = Γ ++ Δ ++ Δ₀ ++ Γ₂ ++ Γ₀} b (scut⊗r-hass Δ₀ f₁ f₁' f₂ g refl)
scut⊗r-hass {Γ₁ = Γ} {Δ} ._ {Δ'} {A₂ = A₂} f₁ f₁' f₂ (ex {Γ = Γ₁} {.(Γ₀ ++ _ ∷ Δ')} {A} {B} b g) refl | inj₂ (inj₁ (Γ₀ , refl , refl))
   rewrite cases++-inj₂ (B ∷ A ∷ Γ₀) (Γ ++ Δ ++ Γ₁) Δ' A₂ = 
   ex {Γ = Γ ++ Δ ++ Γ₁} b (scut⊗r-hass (Γ₁ ++ _ ∷ _ ∷ Γ₀) f₁ f₁' f₂ g refl)
scut⊗r-hass {Γ₁ = Γ} {Δ} {Γ₂} Δ₀ {A₂ = A₂} f₁ f₁' f₂ (ex {Γ = Δ₀} {Δ₁} {A} b g) refl | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
   rewrite cases++-inj₂ [] (Γ ++ Δ ++ Δ₀) (A ∷ Δ₁) A₂ =
   ≡-to-≗ (scut⊗r-ex-fma-cxt Γ₂ f₁ f₁' (ccut (Δ₀ ++ _ ∷ []) f₂ g refl))
   ∙ cong-ex-fma-cxt Γ₂ b (scut⊗r-hass (Δ₀ ++ _ ∷ []) f₁ f₁' f₂ g refl)
scut⊗r-hass {Γ₁ = Γ} {Δ} {Γ₂} _ {Δ'} {A₂ = A₂} f₁ f₁' f₂ (ex {Γ = Γ₁} {Δ'} {B = B} b g) refl | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
   rewrite cases++-inj₂ (B ∷ []) (Γ ++ Δ ++ Γ₁) Δ' A₂ =
   ≡-to-≗ (scut⊗r-ex-cxt-fma Γ₂ f₁ f₁' (ccut Γ₁ f₂ g refl))
   ∙ cong-ex-cxt-fma {Γ = Γ ++ Δ ++ Γ₁} Γ₂ b (scut⊗r-hass Γ₁ f₁ f₁' f₂ g refl)
...
scut-hass Δ₀ {Δ'} {A₂ = A₂} (ex {Γ = Γ} {Δ} {A} {B} b f₁) f₂ g refl
  rewrite cases++-inj₂ (B ∷ A ∷ Δ ++ Δ₀) Γ Δ' A₂ = ex b (scut-hass Δ₀ f₁ f₂ g refl)
...
ccut-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {Δ₁ = Δ₁} {Δ₂} {A₁} {A₂} f₁ f₂ (ex {Γ = Γ} {Δ} {A} {B} b g) r with cases++2 (Δ₀ ++ A₁ ∷ Δ₁) Γ Δ₂ Δ r
ccut-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {Δ₁ = Δ₁} {.(Γ₀ ++ B ∷ A ∷ Δ)} {A₁} {A₂} f₁ f₂ (ex {Γ = .(Δ₀ ++ A₁ ∷ Δ₁ ++ A₂ ∷ Γ₀)} {Δ} {A} {B} b g) refl | inj₁ (Γ₀ , refl , refl)
  rewrite cases++-inj₁ Δ₀ (Δ₁ ++ A₂ ∷ Γ₀) (B ∷ A ∷ Δ) A₁ |
          cases++-inj₁ Δ₀ (Δ₁ ++ Γ₂ ++ Γ₀) (B ∷ A ∷ Δ) A₁ |
          cases++-inj₁ (Δ₀ ++ Γ₁ ++ Δ₁) Γ₀ (B ∷ A ∷ Δ) A₂ =
          ex {Γ = Δ₀ ++ Γ₁ ++ Δ₁ ++ Γ₂ ++ Γ₀} b (ccut-hass Δ₀ f₁ f₂ g refl)
ccut-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {Δ₁ = Δ₁} {.(A ∷ Δ)} {A₁} {A₂} f₁ f₂ (ex {Γ = .(Δ₀ ++ A₁ ∷ Δ₁)} {Δ} {A} {.A₂} b g) refl | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₁ Δ₀ Δ₁ (A₂ ∷ A ∷ Δ) A₁ |
          cases++-inj₂ [] (Δ₀ ++ Γ₁ ++ Δ₁) (A ∷ Δ) A₂ =
          ≡-to-≗ (ccut-ex-fma-cxt2 Δ₀ Γ₂ f₁ (ccut (Δ₀ ++ A₁ ∷ Δ₁ ++ A ∷ []) f₂ g refl))
          ∙ cong-ex-fma-cxt Γ₂ b (ccut-hass Δ₀ {Δ₁ = Δ₁ ++ _ ∷ []} f₁ f₂ g refl)
... | inj₂ (inj₁ (Γ₀ , refl , q)) with cases++2 Δ₀ Γ Δ₁ Γ₀ (sym q)
ccut-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {Δ₁ = .(Γ₀' ++ B ∷ A ∷ Γ₀)} {Δ₂} {A₁} {A₂} f₁ f₂ (ex {Γ = .(Δ₀ ++ A₁ ∷ Γ₀')} {.(Γ₀ ++ A₂ ∷ Δ₂)} {A} {B} b g) refl | inj₂ (inj₁ (Γ₀ , refl , refl)) | inj₁ (Γ₀' , refl , refl)
  rewrite cases++-inj₁ Δ₀ Γ₀' (B ∷ A ∷ Γ₀ ++ A₂ ∷ Δ₂) A₁ |
          cases++-inj₁ Δ₀ Γ₀' (B ∷ A ∷ Γ₀ ++ Γ₂ ++ Δ₂) A₁ |
          cases++-inj₂ (B ∷ A ∷ Γ₀) (Δ₀ ++ Γ₁ ++ Γ₀') Δ₂ A₂ =
          ex {Γ = Δ₀ ++ Γ₁ ++ Γ₀'} b (ccut-hass Δ₀ {Δ₁ = Γ₀' ++ _ ∷ _ ∷ Γ₀} f₁ f₂ g refl)
ccut-hass {Γ₁ = Γ₁} {Γ₂} .(Γ ++ B ∷ A ∷ Γ₀') {Δ₁ = Δ₁} {Δ₂} {A₁} {A₂} f₁ f₂ (ex {Γ = Γ} {.((Γ₀' ++ A₁ ∷ Δ₁) ++ A₂ ∷ Δ₂)} {A} {B} b g) refl | inj₂ (inj₁ (.(Γ₀' ++ A₁ ∷ Δ₁) , refl , refl)) | inj₂ (inj₁ (Γ₀' , refl , refl))
  rewrite cases++-inj₂ (B ∷ A ∷ Γ₀') Γ (Δ₁ ++ A₂ ∷ Δ₂) A₁ |
          cases++-inj₂ (B ∷ A ∷ Γ₀') Γ (Δ₁ ++ Γ₂ ++ Δ₂) A₁ |
          cases++-inj₂ (B ∷ A ∷ Γ₀' ++ Γ₁ ++ Δ₁) Γ Δ₂ A₂ =
          ex b (ccut-hass (Γ ++ _ ∷ _ ∷ Γ₀') f₁ f₂ g refl)
ccut-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {Δ₁ = .(A ∷ Γ₀)} {Δ₂} {A₁} {A₂} f₁ f₂ (ex {Γ = .Δ₀} {.(Γ₀ ++ A₂ ∷ Δ₂)} {A} {.A₁} b g) refl | inj₂ (inj₁ (Γ₀ , refl , refl)) | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₂ [] Δ₀ (A ∷ Γ₀ ++ A₂ ∷ Δ₂) A₁ |
          cases++-inj₂ [] Δ₀ (A ∷ Γ₀ ++ Γ₂ ++ Δ₂) A₁ =
          cong-ex-fma-cxt Γ₁ b (ccut-hass (Δ₀ ++ _ ∷ []) f₁ f₂ g refl)
          ∙ ~ (≡-to-≗ (ccut-ex-fma-cxt2' Δ₀ Γ₁ f₂ (ccut (Δ₀ ++ A ∷ []) f₁ g refl)))
ccut-hass {Γ₁ = Γ₁} {Γ₂} .(Γ ++ B ∷ []) {Δ₁ = .Γ₀} {Δ₂} {A₁} {A₂} f₁ f₂ (ex {Γ = Γ} {.(Γ₀ ++ A₂ ∷ Δ₂)} {.A₁} {B} b g) refl | inj₂ (inj₁ (Γ₀ , refl , refl)) | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
  rewrite cases++-inj₂ (B ∷ []) Γ (Γ₀ ++ A₂ ∷ Δ₂) A₁ |
          cases++-inj₂ (B ∷ []) Γ (Γ₀ ++ Γ₂ ++ Δ₂) A₁ =
          cong-ex-cxt-fma Γ₁ b (ccut-hass Γ f₁ f₂ g refl)
          ∙ ~ (≡-to-≗ (ccut-ex-cxt-fma2' Γ Γ₁ f₂ (ccut Γ f₁ g refl)))
ccut-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {Δ₁ = Δ₁} {Δ₂} {A₁} {A₂} f₁ f₂ (ex {Γ = Γ} {.Δ₂} {.A₂} {B} b g) r | inj₂ (inj₂ (inj₂ (refl , refl , q))) with cases++ Γ Δ₀ [] (_ ∷ Δ₁) q
... | inj₁ (Γ₀ , p , p') = ⊥-elim ([]disj∷ Γ₀ p')
ccut-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {Δ₁ = .[]} {Δ₂} {A₁} {A₂} f₁ f₂ (ex {Γ = .(Δ₀ ++ [])} {Δ₂} {A₂} {.A₁} b g) refl | inj₂ (inj₂ (inj₂ (refl , refl , refl))) | inj₂ ([] , refl , refl)
  rewrite cases++-inj₂ [] Δ₀ (A₂ ∷ Δ₂) A₁ =
  ≡-to-≗ (ccut-ex-cxt-cxt2 Δ₀ Γ₂ f₁ (ccut Δ₀ f₂ g refl))
  ∙ ex-cxt-cxt≗ Γ₂ Γ₁ _ 
  ∙ cong-ex-cxt-cxt2 Γ₂ Γ₁ b (~ (ccut-hass Δ₀ f₂ f₁ g refl))
  ∙ ~ (≡-to-≗ (ccut-ex-cxt-cxt2' Δ₀ Γ₁ f₂ (ccut (Δ₀ ++ A₂ ∷ []) f₁ g refl)))
ccut-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {Δ₁ = .(Γ₀ ++ B ∷ [])} {Δ₂} {.x} {A₂} f₁ f₂ (ex {Γ = .(Δ₀ ++ x ∷ Γ₀)} {Δ₂} {A₂} {B} b g) refl | inj₂ (inj₂ (inj₂ (refl , refl , refl))) | inj₂ (x ∷ Γ₀ , refl , refl)
  rewrite cases++-inj₁ Δ₀ Γ₀ (B ∷ A₂ ∷ Δ₂) x | cases++-inj₂ (B ∷ []) (Δ₀ ++ Γ₁ ++ Γ₀) Δ₂ A₂ =
  ≡-to-≗ (ccut-ex-cxt-fma2 Δ₀ Γ₂ f₁ (ccut (Δ₀ ++ x ∷ Γ₀) f₂ g refl))
  ∙ cong-ex-cxt-fma {Γ = Δ₀ ++ Γ₁ ++ Γ₀} Γ₂ b (ccut-hass Δ₀ f₁ f₂ g refl)
...
cong-scut⊗r {Γ = Γ}{f = f}{g}{f'}{g'} (⊗l h) p q =
  ...
  qed≗
cong-scut⊗r {Γ = Γ} {Δ} (ex {Γ = Λ} b h) p q =
  ex {Γ = Γ ++ Δ ++ Λ} b (cong-scut⊗r h p q)
...
cong-ccut1 Δ₀ {Δ'} (ex {Γ = Γ} {Δ} b g) r p with cases++2 Δ₀ Γ Δ' Δ r
cong-ccut1 {Γ = Γ} Δ₀ {.(Γ₀ ++ _ ∷ _ ∷ Δ)} (ex {Γ = .(Δ₀ ++ _ ∷ Γ₀)} {Δ} b g) r p | inj₁ (Γ₀ , refl , refl) = ex {Γ = Δ₀ ++ Γ ++ Γ₀} b (cong-ccut1 Δ₀ g refl p)
... | inj₂ (inj₁ (Γ₀ , refl , refl)) = ex b (cong-ccut1 (Γ ++ _ ∷ _ ∷ Γ₀) g refl p)
cong-ccut1 {Γ = Γ} Δ₀ {.(_ ∷ Δ)} (ex {Γ = .Δ₀} {Δ} b g) r p | inj₂ (inj₂ (inj₁ (refl , refl , refl))) = cong-ex-fma-cxt Γ b (cong-ccut1 (Δ₀ ++ _ ∷ []) g refl p)
cong-ccut1 {Γ = Γ₁} .(Γ ++ _ ∷ []) {Δ'} (ex {Γ = Γ} {.Δ'} b g) r p | inj₂ (inj₂ (inj₂ (refl , refl , refl))) = cong-ex-cxt-fma Γ₁ b (cong-ccut1 Γ g refl p)
...
cong-ccut2 Δ₀ {Δ'} r (ex {Γ = Γ} {Δ} b p) with cases++2 Δ₀ Γ Δ' Δ r
cong-ccut2 {Γ = Γ} Δ₀ r (ex {Γ = _} {_} b p) | inj₁ (Γ₀ , refl , refl) =
  ex {Γ = Δ₀ ++ Γ ++ Γ₀} b (cong-ccut2 Δ₀ refl p)
... | inj₂ (inj₁ (Γ₀ , refl , refl)) = ex {Γ = Γ} b (cong-ccut2 (Γ ++ _ ∷ _ ∷ Γ₀) refl p)
cong-ccut2 {Γ = Γ} Δ₀ r (ex {Γ = _} {_} b p) | inj₂ (inj₂ (inj₁ (refl , refl , refl))) =
  cong-ex-fma-cxt Γ b (cong-ccut2 (Δ₀ ++ _ ∷ []) refl p)
cong-ccut2 {Γ = Γ} ._ r (ex {Γ = Γ₁} {_} b p) | inj₂ (inj₂ (inj₂ (refl , refl , refl))) =
  cong-ex-cxt-fma Γ b (cong-ccut2 Γ₁ refl p)
...
cong-ccut2 {Γ = Γ} ._ {A = A₁} refl (exex {Γ₁ = Γ₁} {Γ₂} {Γ₃} {A} {B} {A'}) | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₂ (B ∷ A ∷ Γ₂) Γ₁ (A' ∷ Γ₃) A₁ |
          ...
          cases++-inj₂ [] (Γ₁ ++ A ∷ B ∷ Γ₂) (A' ∷ Γ₃) A₁ = ex-fma-cxt-ex Γ _
cong-ccut2 {Γ = Γ} ._ {Δ'} {A = A₁} refl (exex {Γ₁ = Γ₁} {Γ₂} {Γ₃} {A} {B} {A'} {B'}) | inj₂ (inj₂ (inj₂ (refl , refl , refl))) 
  rewrite cases++-inj₂ (B ∷ A ∷ Γ₂ ++ B' ∷ []) Γ₁ Δ' A₁ |
          ...
          cases++-inj₂ (B' ∷ []) (Γ₁ ++ A ∷ B ∷ Γ₂) Δ' A₁ = ex-cxt-fma-ex Γ _
...
cong-ccut2 {Γ = Γ} Δ₀ {A = A₁} refl (exex {Γ₁ = Γ₁} {Γ₂} {Γ₃} {A} {B} {A'}{B'} ) | inj₁ (.(_ ∷ Γ₂) , refl , refl) | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₂ [] Δ₀ (A ∷ Γ₂ ++ B' ∷ A' ∷ Γ₃) A₁ |
          ...
          cases++-inj₁ (Δ₀ ++ A ∷ []) Γ₂ (B' ∷ A' ∷ Γ₃) A₁ = ~ ex-ex-fma-cxt Γ _
cong-ccut2 {Γ = Γ} ._ {A = A₁} refl (exex {Γ₁ = Γ₁} {Γ₃ = Γ₃} {A} {B} {A'}{B'}) | inj₁ (Γ₀ , refl , refl) | inj₂ (inj₂ (inj₂ (refl , refl , refl))) 
  rewrite cases++-inj₂ (B ∷ []) Γ₁ (Γ₀ ++ B' ∷ A' ∷ Γ₃) A₁ |
          ...
          cases++-inj₁ Γ₁ (B ∷ Γ₀) (B' ∷ A' ∷ Γ₃) A₁ = ~ ex-ex-cxt-fma Γ _
cong-ccut2 {Γ = Γ} Δ₀ {Δ'} r (exuf {Γ₁} {Δ} {A'} {A} {B}) with cases∷ Δ₀ r
cong-ccut2 {Γ = Γ} .[] {A = A₁} {f = f} refl (exuf {Γ₁} {Δ} {A'} {A} {B}) | inj₁ (refl , refl , refl) = ~ scut-ex _ f
...
cong-ccut2 {Γ = Γ} Δ₀ {_} {A₁} refl (ex⊗l {Γ₁} {Δ} {B' = B'} {A} {B}) | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₂ [] Δ₀ (A ∷ Δ) A₁ = ex-fma-cxt-⊗l Δ₀ Γ _
cong-ccut2 {Γ = Γ} _ {Δ'} {A₁} refl (ex⊗l {Γ₁} {Δ} {B' = B'} {A} {B}) | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
  rewrite cases++-inj₂ (B ∷ []) Γ₁ Δ' A₁ = ex-cxt-fma-⊗l Γ₁ Γ _
...
cong-ccut2 {Γ = Γ₁} Δ₀ {Δ'} {A} r (ex-iso {Γ = Γ} {Δ} {A₁} {B} b) with cases++2 Δ₀ Γ Δ' Δ r
cong-ccut2 {Γ = Γ₁} Δ₀ {_} {A} refl (ex-iso {Γ = Γ} {Δ} {A₁} {B} b) | inj₁ (Γ₀ , refl , refl)
  rewrite cases++-inj₁ Δ₀ Γ₀ (B ∷ A₁ ∷ Δ) A = ex-iso {Γ = Δ₀ ++ Γ₁ ++ Γ₀} b
cong-ccut2 {Γ = Γ₁} _ {Δ'} {A} refl (ex-iso {Γ = Γ} {Δ} {A₁} {B} b) | inj₂ (inj₁ (Γ₀ , refl , refl))
  rewrite cases++-inj₂ (B ∷ A₁ ∷ Γ₀) Γ Δ' A = ex-iso b
cong-ccut2 {Γ = Γ₁} Δ₀ {_}{A} refl (ex-iso {Γ = Γ} {Δ} {A₁} {B} b) | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₂ (B ∷ []) Δ₀ Δ A = ex-fma-cxt-iso2 Γ₁
cong-ccut2 {Γ = Γ₁} _ {Δ'} {A} refl (ex-iso {Γ = Γ} {Δ} {A₁} {B} b) | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
...
cong-ccut2 {Γ = Γ} _ {Δ'} {A} refl (ex-braid {Γ = Γ₁} {Δ} {A₁} {B} {C}) | inj₂ (inj₁ ([] , refl , refl))
  rewrite cases++-inj₂ (B ∷ []) (Γ₁ ++ C ∷ []) Δ' A |
          ...
          cases++-inj₂ (C ∷ []) Γ₁ (B ∷ Δ') A |
          cases++-inj₂ (B ∷ []) Γ₁ (C ∷ Δ') A |
          cases++-inj₁ Γ₁ [] (C ∷ B ∷ Δ') A = ex-cxt-fma-braid Γ _
...
scutscut-vass (ex b f) g h = ex b (scutscut-vass f g h)

scutscut⊗r-vass f f' ax h = refl
scutscut⊗r-vass {Γ = Γ} {Γ'} f f' (ex {Γ = Γ₁} b g) h =
  ex {Γ = Γ ++ Γ' ++ Γ₁} b (scutscut⊗r-vass f f' g h)
...
scutscut⊗r-vass {Γ = Γ'} {Δ'} f f' (⊗r {Γ = Γ} {Δ} g g') (ex {Γ = Γ₁} b h) =
  ex {Γ = Γ' ++ Δ' ++ Γ ++ Δ ++ Γ₁} b (scutscut⊗r-vass f f' (⊗r g g') h)
...
ccutscut⊗r-vass1 {Γ = Γ₁} {Δ2 = Δ2} Δ₀ {Δ'} {A = A} f g1 g2 (ex {Γ = Γ} {Δ} {A₁} {B = B} b h) refl 
  rewrite cases++-inj₁ Δ₀ Δ' Δ2 A |
          cases++-inj₁ Δ₀ (Δ' ++ Δ2 ++ Γ) (B ∷ A₁ ∷ Δ) A =
  ex {Γ = Δ₀ ++ Γ₁ ++ Δ' ++ Δ2 ++ Γ} b (ccutscut⊗r-vass1 Δ₀ f g1 g2 h refl)
...
ccutscut⊗r-vass2 {Γ = Γ₁} {Δ = Δ} Δ₀ {Δ'} {A = A} f g1 g2 (ex {Γ = Γ} {Δ₁} {A₁} {B} b h) refl
  rewrite cases++-inj₁ (Δ ++ Δ₀) (Δ' ++ Γ) (B ∷ A₁ ∷ Δ₁) A =
  ex {Γ = Δ ++ Δ₀ ++ Γ₁ ++ Δ' ++ Γ} b (ccutscut⊗r-vass2 Δ₀ f g1 g2 h refl)
...
ccutscut-vass Δ₀ {Δ'} f (ex {Γ = Γ} {Δ} b g) h eq with cases++2 Δ₀ Γ Δ' Δ eq
ccutscut-vass {Γ = Γ} Δ₀ {.(Γ₀ ++ B ∷ A₁ ∷ Δ)} {Δ''} {A} f (ex {Γ = .(Δ₀ ++ A ∷ Γ₀)} {Δ} {A₁} {B} b g) h refl | inj₁ (Γ₀ , refl , refl)
  rewrite cases++-inj₁ Δ₀ Γ₀ (B ∷ A₁ ∷ Δ ++ Δ'') A =
  ex {Γ = Δ₀ ++ Γ ++ Γ₀} b (ccutscut-vass Δ₀ f g h refl)
ccutscut-vass .(Γ ++ B ∷ A₁ ∷ Γ₀) {Δ'} {Δ''} {A} f (ex {Γ = Γ} {.(Γ₀ ++ A ∷ Δ')} {A₁} {B} b g) h refl | inj₂ (inj₁ (Γ₀ , refl , refl))
  rewrite cases++-inj₂ ( B ∷ A₁ ∷ Γ₀) Γ (Δ' ++ Δ'') A =
  ex b (ccutscut-vass (Γ ++ _ ∷ _ ∷ Γ₀) f g h refl)
ccutscut-vass {Γ = Γ} Δ₀ {.(A₁ ∷ Δ)} {Δ''} {A} f (ex {Γ = Δ₀} {Δ} {A₁} b g) h refl | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₂ [] Δ₀ (A₁ ∷ Δ ++ Δ'') A =
  cong-ex-fma-cxt Γ b (ccutscut-vass (Δ₀ ++ _ ∷ []) f g h refl)
  ∙ ≡-to-≗ (sym (scut-ex-fma-cxt Γ _ _))
ccutscut-vass {Γ = Γ₁} .(Γ ++ B ∷ []) {Δ'} {Δ''} {A} f (ex {Γ = Γ} {Δ'} {B = B} b g) h refl | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
  rewrite cases++-inj₂ (B ∷ []) Γ (Δ' ++ Δ'') A =
  cong-ex-cxt-fma Γ₁ b (ccutscut-vass Γ f g h refl)
...
ccutccut-vass Γ₀ Δ₀ {Δ'} f g (ex {Γ = Γ} {Δ} b h) r with cases++2 Δ₀ Γ Δ' Δ r
ccutccut-vass {Γ = Γ} Γ₀ Δ₀ {.(Λ₀ ++ B ∷ A₁ ∷ Δ)} {Γ'} {A} f g (ex {Γ = .(Δ₀ ++ _ ∷ Λ₀)} {Δ} {A₁} {B} b h) refl | inj₁ (Λ₀ , refl , refl)
  rewrite cases++-inj₁ (Δ₀ ++ Γ₀) (Γ' ++ Λ₀) (B ∷ A₁ ∷ Δ) A =
  ex {Γ = Δ₀ ++ Γ₀ ++ Γ ++ Γ' ++ Λ₀} b (ccutccut-vass Γ₀ Δ₀ f g h refl)
ccutccut-vass Γ₀ .(Γ ++ B ∷ A₁ ∷ Λ₀) {Δ'} {Γ'} {A} f g (ex {Γ = Γ} {.(Λ₀ ++ _ ∷ Δ')} {A₁} {B} b h) refl | inj₂ (inj₁ (Λ₀ , refl , refl))
  rewrite cases++-inj₂ (B ∷ A₁ ∷ Λ₀ ++ Γ₀) Γ (Γ' ++ Δ') A =
  ex b (ccutccut-vass Γ₀ (Γ ++ _ ∷ _ ∷ Λ₀) f g h refl)
ccutccut-vass {Γ = Γ} Γ₀ Δ₀ {.(_ ∷ Δ)} {Γ'} {A = A} f g (ex {Γ = Δ₀} {Δ} {A₁} b h) refl | inj₂ (inj₂ (inj₁ (refl , refl , refl))) =
  proof≗
    ccut (Δ₀ ++ Γ₀) f (ex-fma-cxt (Γ₀ ++ _ ∷ Γ') b (ccut (Δ₀ ++ _ ∷ []) g h refl)) refl
  ≗〈 ≡-to-≗ (cong (λ z → ccut (Δ₀ ++ Γ₀) f z refl) (ex-fma-cxt++ Γ₀ (_ ∷ Γ') b (ccut (Δ₀ ++ _ ∷ []) g h refl))) 〉 
    ccut (Δ₀ ++ Γ₀) f (ex-fma-cxt {Γ = Δ₀ ++ Γ₀ ++ _ ∷ []} Γ' b (ex {Γ = Δ₀ ++ Γ₀} b (ex-fma-cxt Γ₀ b (ccut (Δ₀ ++ _ ∷ []) g h refl)))) refl
  ≗〈 ≡-to-≗ (ccut-ex-fma-cxt2 (Δ₀ ++ Γ₀) Γ' f _) 〉 
    ex-fma-cxt {Γ = Δ₀ ++ Γ₀ ++ Γ} Γ' b (ccut (Δ₀ ++ Γ₀) f (ex {Γ = Δ₀ ++ Γ₀} b (ex-fma-cxt Γ₀ b (ccut (Δ₀ ++ _ ∷ []) g h refl))) refl)
  ≗〈 cong-ex-fma-cxt Γ' b (≡-to-≗ lem) 〉 
    ex-fma-cxt {Γ = Δ₀ ++ Γ₀ ++ Γ} Γ' b (ex-fma-cxt {Γ = Δ₀ ++ Γ₀} Γ b (ex-fma-cxt Γ₀ b (ccut (Δ₀ ++ _ ∷ Γ₀) f (ccut (Δ₀ ++ _ ∷ []) g h refl) refl)))
  ≗〈 cong-ex-fma-cxt Γ' b (≡-to-≗ (sym (ex-fma-cxt++ Γ₀ Γ b _))) 〉 
    ex-fma-cxt {Γ = Δ₀ ++ Γ₀ ++ Γ} Γ' b (ex-fma-cxt (Γ₀ ++ Γ) b (ccut (Δ₀ ++ _ ∷ Γ₀) f (ccut (Δ₀ ++ _ ∷ []) g h refl) refl))
  ≗〈 ≡-to-≗ (sym (ex-fma-cxt++ (Γ₀ ++ Γ) Γ' b _)) 〉 
    ex-fma-cxt (Γ₀ ++ Γ ++ Γ') b (ccut (Δ₀ ++ _ ∷ Γ₀) f (ccut (Δ₀ ++ _ ∷ []) g h refl) refl)
  ≗〈 cong-ex-fma-cxt (Γ₀ ++ Γ ++ Γ') b (ccutccut-vass Γ₀ (Δ₀ ++ _ ∷ []) f g h refl) 〉 
    _
  qed≗
  where
    lem : ccut (Δ₀ ++ Γ₀) f (ex {Γ = Δ₀ ++ Γ₀} b (ex-fma-cxt Γ₀ b (ccut (Δ₀ ++ _ ∷ []) g h refl))) refl
          ≡ ex-fma-cxt {Γ = Δ₀ ++ Γ₀} Γ b (ex-fma-cxt Γ₀ b (ccut (Δ₀ ++ _ ∷ Γ₀) f (ccut (Δ₀ ++ _ ∷ []) g h refl) refl))
    lem rewrite cases++-inj₂ [] (Δ₀ ++ Γ₀) (A₁ ∷ Γ' ++ Δ) A =
      cong (ex-fma-cxt Γ b) (ccut-ex-fma-cxt2' Δ₀ Γ₀ f _)
ccutccut-vass {Γ = Γ₁} Γ₀ .(Γ ++ _ ∷ []) {Δ'} {Γ'} {A = A} f g (ex {Γ = Γ} {Δ'} {_}{B} b h) refl | inj₂ (inj₂ (inj₂ (refl , refl , refl))) =
  proof≗
    ccut (Γ ++ _ ∷ Γ₀) f (ex-cxt-fma (Γ₀ ++ _ ∷ Γ') b (ccut Γ g h refl)) refl
  ≗〈 ≡-to-≗ (cong (λ z → ccut (Γ ++ _ ∷ Γ₀) f z refl) (ex-cxt-fma++ Γ₀ (_ ∷ Γ') b _)) 〉
    ccut (Γ ++ _ ∷ Γ₀) f (ex-cxt-fma Γ₀ b (ex {Γ = Γ ++ Γ₀} b (ex-cxt-fma {Γ = Γ ++ Γ₀ ++ _ ∷ []} Γ' b (ccut Γ g h refl)))) refl
  ≗〈 ≡-to-≗ (ccut-ex-cxt-fma2' Γ Γ₀ f _)  〉
    ex-cxt-fma {Γ = Γ}{Γ₁ ++ Γ' ++ Δ'} Γ₀ b (ccut (Γ ++ Γ₀ ++ _ ∷ []) f (ex {Γ = Γ ++ Γ₀} b (ex-cxt-fma {Γ = Γ ++ Γ₀ ++ _ ∷ []} Γ' b (ccut Γ g h refl))) refl)
  ≗〈 cong-ex-cxt-fma Γ₀ b (≡-to-≗ lem) 〉
    ex-cxt-fma {Γ = Γ} {Γ₁ ++ Γ' ++ Δ'} Γ₀ b (ex-cxt-fma {Γ = Γ ++ Γ₀} Γ₁ b (ex-cxt-fma {Γ = Γ ++ Γ₀ ++ Γ₁} Γ' b (ccut (Γ ++ Γ₀) f (ccut Γ g h refl) refl)))
  ≗〈 cong-ex-cxt-fma Γ₀ b (≡-to-≗ (sym (ex-cxt-fma++ {Γ = Γ ++ Γ₀} Γ₁ Γ' b _))) 〉
    ex-cxt-fma {Γ = Γ} {Γ₁ ++ Γ' ++ Δ'} Γ₀ b (ex-cxt-fma {Γ = Γ ++ Γ₀} (Γ₁ ++ Γ') b (ccut (Γ ++ Γ₀) f (ccut Γ g h refl) refl))
  ≗〈 ≡-to-≗ (sym (ex-cxt-fma++ Γ₀ (Γ₁ ++ Γ') b _)) 〉
    ex-cxt-fma (Γ₀ ++ Γ₁ ++ Γ') b (ccut (Γ ++ Γ₀) f (ccut Γ g h refl) refl)
  ≗〈 cong-ex-cxt-fma (Γ₀ ++ Γ₁ ++ Γ') b (ccutccut-vass Γ₀ Γ f g h refl) 〉
    _
  qed≗
  where
    lem : ccut (Γ ++ Γ₀ ++ _ ∷ []) f (ex {Γ = Γ ++ Γ₀} b (ex-cxt-fma {Γ = Γ ++ Γ₀ ++ _ ∷ []} Γ' b (ccut Γ g h refl))) refl
          ≡ ex-cxt-fma {Γ = Γ ++ Γ₀} Γ₁ b (ex-cxt-fma {Γ = Γ ++ Γ₀ ++ Γ₁} Γ' b (ccut (Γ ++ Γ₀) f (ccut Γ g h refl) refl))
    lem rewrite cases++-inj₂ (B ∷ []) (Γ ++ Γ₀) (Γ' ++ Δ') A =
                cong (ex-cxt-fma {Γ = Γ ++ Γ₀} Γ₁ b) (ccut-ex-cxt-fma2 (Γ ++ Γ₀) Γ' f _)
