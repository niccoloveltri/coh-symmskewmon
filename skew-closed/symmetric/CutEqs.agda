...
scut-ex Ir = refl
scut-ex {Δ₁ = Δ₁} {Δ₂} (⊸l {Δ = Δ} f f') =
  ⊸l refl (scut-ex f') ∙ ~ ex⊸l₂ {_}{Δ ++ Δ₁}
scut-ex {Γ = Γ} {Δ} (Il f) = 
  Il (scut-ex f) ∙ ~ exIl {Γ = Γ ++ Δ} 
scut-ex {Γ = Γ} (⊸r f) = refl
...
scut⊸r-ex-fma-cxt : ∀{S Γ Δ₁ Δ₂} Λ {A' B' A C}
  → (f : S ∣ Γ ∷ʳ A' ⊢ B') (g : just (A' ⊸ B') ∣ Δ₁ ++ A ∷ Λ ++ Δ₂ ⊢ C)
  → scut (⊸r f) (ex-fma-cxt {Γ = Δ₁}{Δ₂} Λ g) ≡ ex-fma-cxt {Γ = Γ ++ Δ₁} Λ (scut (⊸r f) g)
scut⊸r-ex-fma-cxt [] f g = refl
scut⊸r-ex-fma-cxt {Δ₁ = Δ₁} (x ∷ Λ) f g =
  scut⊸r-ex-fma-cxt {Δ₁ = Δ₁ ++ x ∷ []} Λ f (ex g)

scut⊸r-ex-cxt-fma : ∀{S Γ Δ₁ Δ₂} Λ {A' B' A C}
  → (f : S ∣ Γ ∷ʳ A' ⊢ B') (g : just (A' ⊸ B') ∣ Δ₁ ++ Λ ++ A ∷ Δ₂ ⊢ C)
  → scut (⊸r f) (ex-cxt-fma {Γ = Δ₁}{Δ₂} Λ g) ≡ ex-cxt-fma {Γ = Γ ++ Δ₁} Λ (scut (⊸r f) g)
scut⊸r-ex-cxt-fma [] f g = refl
scut⊸r-ex-cxt-fma {Γ = Γ}{Δ₁} (x ∷ Λ) f g =
  cong (ex {Γ = Γ ++ Δ₁}) (scut⊸r-ex-cxt-fma {Δ₁ = Δ₁ ++ x ∷ []} Λ f g)
...
ccut-ex {Γ₁ = Γ₁} Δ₀ {Δ'} f (⊸l {Γ = Γ} {Δ} g g₁) eq with cases++ Δ₀ Γ Δ' Δ eq
... | inj₁ (Γ₀ , refl , refl) = 
  ⊸l (ccut-ex Δ₀ f g refl) refl ∙ ~ ex⊸l₁ {Γ₁ = Δ₀ ++ Γ₁}
... | inj₂ (Γ₀ , refl , refl) = 
  ⊸l refl (ccut-ex Γ₀ f g₁ refl) ∙ ~ (ex⊸l₂ {Δ₁ = Γ₀ ++ Γ₁})
ccut-ex {Γ₁ = Γ₁} Δ₀ f (Il g) eq =
  Il (ccut-ex Δ₀ f g eq) ∙ ~ exIl {Γ = Δ₀ ++ Γ₁}
ccut-ex {Γ₁ = Γ₁} Δ₀ f (⊸r g) refl = 
  ⊸r (ccut-ex Δ₀ f g refl) ∙ ~ ex⊸r {Γ = Δ₀ ++ Γ₁}
...
ccut-unit {_}{Γ} Δ₀ (⊸l {Γ = Γ'}{Δ} g g') r with cases++ Δ₀ Γ' Γ Δ r
ccut-unit Δ₀ (⊸l {Γ = Γ}{Δ} g g') refl | inj₁ (Γ₀ , refl , refl) = 
  cong₂ (⊸l {Γ = Γ}{Δ}) (ccut-unit Δ₀ g refl) refl
ccut-unit ._ (⊸l {Γ = Γ}{Δ} g g') refl | inj₂ (Δ₀ , refl , refl) =
  cong₂ (⊸l {Γ = Γ}) refl (ccut-unit Δ₀ g' refl)
ccut-unit Δ₀ (Il g) refl = cong Il (ccut-unit Δ₀ g refl)
ccut-unit Δ₀ (⊸r g) refl = cong ⊸r (ccut-unit Δ₀ g refl)
...
Il-1-scutIr (⊸r f) = cong ⊸r (Il-1-scutIr f)
Il-1-scutIr (Il f) = refl

-- ====================================================================

-- the cut rules commute with Il-1 
...
ccutIl-1 Δ₀ f (⊸r g) refl = cong ⊸r (ccutIl-1 Δ₀ f g refl)
...
scutIl-1 (⊸r f) ax = refl
scutIl-1 {Γ} (⊸r f) (ex {Γ = Γ₁} g) = cong (ex {Γ = Γ ++ Γ₁}) (scutIl-1 (⊸r f) g)
scutIl-1 (⊸r f) (⊸r g) = cong ⊸r (scutIl-1 (⊸r f) g)
scutIl-1 {Γ} (⊸r f) (⊸l {Γ = Γ₁} g h) =
  trans (scutIl-1 (ccut Γ g f refl) h)
    (cong (λ x → scut {Γ = Γ ++ Γ₁} x h) (ccutIl-1 Γ g f refl))

-- ====================================================================

-- some lemmata about scut and ⊸r

scut⊸rex : {S : Stp} {Γ Δ Δ' : Cxt}{A B A' B' C : Fma}
  → {f : S ∣ Γ ++ A' ∷ B' ∷ Δ ∷ʳ A ⊢ B}
  → (g : just (A ⊸ B) ∣ Δ' ⊢ C)
  → scut {Γ = Γ ++ _ ∷ _ ∷ Δ} (⊸r (ex f)) g ≗ ex (scut {Γ = Γ ++ _ ∷ _ ∷ Δ} (⊸r f) g)
scut⊸rex ax = ~ ex⊸r
scut⊸rex {Γ = Γ} {Δ} (ex {Γ = Γ₁} {Δ₁} g) =
  ex {Γ = Γ ++ _ ∷ _ ∷ Δ ++ Γ₁} (scut⊸rex g) ∙ exex {Γ₁ = Γ}{Δ ++ Γ₁} 
scut⊸rex {Γ = Γ} {Δ} {Δ'} (⊸r g) =
  ⊸r (scut⊸rex g) ∙ ~ ex⊸r
scut⊸rex {Γ = Γ} {Δ} {A = A} {A' = A'} {B'} (⊸l g g₁)
  rewrite cases++-inj₂ (B' ∷ A' ∷ Δ) Γ [] A = refl 

scut⊸ruf : {Γ Δ : Cxt}{A A' B C : Fma}
  → {f : just A' ∣ Γ ∷ʳ A ⊢ B}
  → (g : just (A ⊸ B) ∣ Δ ⊢ C)
  → scut (⊸r (uf f)) g ≗ uf (scut (⊸r f) g)
scut⊸ruf ax = ⊸ruf
scut⊸ruf {Γ} (ex {Γ = Γ₁} g) = ex (scut⊸ruf g) ∙ exuf {Γ = Γ ++ Γ₁}
scut⊸ruf {Γ} {Δ} (⊸r g) = ⊸r (scut⊸ruf g) ∙ ⊸ruf {Γ = Γ ++ Δ}
scut⊸ruf (⊸l g g₁) = refl
...
scut⊸rIl ax = ⊸rIl
scut⊸rIl {Γ} (ex {Γ = Γ₁} g) = ex {Γ = Γ ++ Γ₁} (scut⊸rIl g) ∙ exIl {Γ = Γ ++ Γ₁}
scut⊸rIl {Γ} {Δ} (⊸r g) = ⊸r (scut⊸rIl g) ∙ ⊸rIl {Γ = Γ ++ Δ}
scut⊸rIl (⊸l h h₁) = refl

scut⊸r⊸l : {Γ Δ Δ' : Cxt}{A A' B B' C : Fma}
  → {f : nothing ∣ Γ ⊢ A}{g : just B ∣ Δ ∷ʳ A' ⊢ B'}
  → (h : just (A' ⊸ B') ∣ Δ' ⊢ C)
  → scut {Γ = Γ ++ Δ} (⊸r (⊸l f g)) h ≗ ⊸l f (scut (⊸r g) h)
scut⊸r⊸l ax = ⊸r⊸l
scut⊸r⊸l {Γ} {Δ} (ex {Γ = Γ₁} h) = ex {Γ = Γ ++ Δ ++ Γ₁} (scut⊸r⊸l h) ∙ ex⊸l₂ {Δ₁ = Δ ++ Γ₁}
scut⊸r⊸l {Γ} {Δ} {Δ'} (⊸r h) = ⊸r (scut⊸r⊸l h) ∙ ⊸r⊸l {Δ = Δ ++ Δ'}
scut⊸r⊸l {Γ} {Δ} {A' = A'} (⊸l h h₁) rewrite cases++-inj₂ Δ Γ [] A' = refl
...
scut⊸r (uf f) g =
  uf (scut⊸r f g) ∙ ~ ⊸ruf
scut⊸r (ex f) g = ex (scut⊸r f g) ∙ ex⊸r
scut⊸r Ir g = refl
scut⊸r (Il f) g =
  Il (scut⊸r f g) ∙ ~ ⊸rIl
scut⊸r (⊸r f) g = refl
...
cong-scut1 (⊸l p q) = ⊸l p (cong-scut1 q)
cong-scut1 (Il p) = Il (cong-scut1 p)
cong-scut1 {h = ax} (⊸r p) = ⊸r p
cong-scut1 {Γ = Γ} {h = ex {Γ = Γ₁} h} (⊸r p) = ex {Γ = Γ ++ Γ₁} (cong-scut1 {h = h} (⊸r p))
cong-scut1 {h = ⊸r h} (⊸r p) = ⊸r (cong-scut1 {h = h} (⊸r p))
cong-scut1 {Γ = Γ} {h = ⊸l {Γ₁} h h₁} (⊸r p) = 
  cong-scut1 {Γ = Γ ++ Γ₁} {h = h₁} (cong-ccut2 Γ refl p)
cong-scut1 {h = h} axI = (~ (IlIl-1 h)) ∙ Il (≡-to-≗ (Il-1-scutIr h))
cong-scut1 {h = ax} ax⊸ = ax⊸
cong-scut1 {h = ex h} ax⊸ = ex (cong-scut1 {h = h} ax⊸)
cong-scut1 {h = ⊸r h} ax⊸ = ⊸r (cong-scut1 {h = h} ax⊸)
cong-scut1 {h = ⊸l h h₁} ax⊸ = ⊸l (~ ≡-to-≗ (scut-unit h)) refl
cong-scut1 (ex p) = ex (cong-scut1 p) 
cong-scut1 exex = exex
cong-scut1 exuf = exuf
cong-scut1 exIl = exIl
cong-scut1 {h = h} ex⊸r = ~ scut⊸rex h
...
cong-scut2 Ir {f}{g} p = 
    ...
    scut Ir g
    qed≗
cong-scut2 (⊸r h) refl = refl
cong-scut2 (⊸r h) (~ p) = ~ cong-scut2 (⊸r h) p
cong-scut2 (⊸r h) (p ∙ p₁) = cong-scut2 (⊸r h) p ∙ cong-scut2 (⊸r h) p₁
cong-scut2 (⊸r h) (⊸r p) = ⊸r (cong-scut2 (⊸r h) p)
cong-scut2 {Γ = Γ} (⊸r h) (⊸l {Γ₁} p p₁) = 
  cong-scut1 {Γ = Γ ++ Γ₁} (cong-ccut1 Γ h refl p)
  ∙ cong-scut2 (ccut Γ _ h refl) p₁
cong-scut2 {Γ = Γ} (⊸r h) ax⊸ = 
  ⊸r (≡-to-≗ (sym (trans (scut-unit (ccut Γ (uf ax) h refl)) (ccut-unit Γ h refl))))
cong-scut2 {Γ = Γ} (⊸r h) ⊸r⊸l = ~ scut⊸r (ccut Γ _ h refl) _
cong-scut2 {Γ = Γ} (⊸r h) (ex {Γ = Γ₁} p) = ex {Γ = Γ ++ Γ₁} (cong-scut2 (⊸r h) p)
cong-scut2 {Γ = Γ} (⊸r h) (exex {Γ₁ = Γ₁}) = exex {Γ₁ = Γ ++ Γ₁}
cong-scut2 {Γ = Γ} (⊸r h) (ex⊸r {Γ = Γ₁}) = ex⊸r {Γ = Γ ++ Γ₁}
cong-scut2 {Γ = Γ} (⊸r h) (ex⊸l₁ {Γ₁} {f = f}) = ~ cong-scut1 (ccut-ex Γ f h refl)
cong-scut2 {Γ = Γ} (⊸r h) (ex⊸l₂ {f = f} {g}) = ~ scut-ex (ccut Γ f h refl) 
cong-scut2 {Γ = Γ} (⊸r h) (ex-iso {Γ = Γ₁}) = ex-iso {Γ = Γ ++ Γ₁}
cong-scut2 {Γ = Γ} (⊸r h) (ex-braid {Γ = Γ₁}) = ex-braid {Γ = Γ ++ Γ₁}
cong-scut2 (Il h) p = Il (cong-scut2 h p)
cong-scut2 (⊸l h h') p = ⊸l refl (cong-scut2 h' p)
...
cong-ccut1 Δ₀ {Δ'} (⊸l {Γ = Γ}{Δ} g g') r p with cases++ Δ₀ Γ Δ' Δ r
cong-ccut1 Δ₀ (⊸l g g') r p | inj₁ (Γ₀ , refl , refl) = ⊸l (cong-ccut1 Δ₀ g refl p) refl
cong-ccut1 ._ (⊸l g g') r p | inj₂ (Γ₀ , refl , refl) = ⊸l refl (cong-ccut1 Γ₀ g' refl p)
...
cong-ccut2 Δ₀ {Δ'} r (⊸l {Γ = Γ}{Δ} p p') with cases++ Δ₀ Γ Δ' Δ r
cong-ccut2 Δ₀ r (⊸l p p') | inj₁ (Γ₀ , refl , refl) = ⊸l (cong-ccut2 Δ₀ refl p) p'
cong-ccut2 ._ r (⊸l p p') | inj₂ (Γ₀ , refl , refl) = ⊸l p (cong-ccut2 Γ₀ refl p')
cong-ccut2 Δ₀ r axI = ⊥-elim ([]disj∷ Δ₀ r)
cong-ccut2 Δ₀ r ax⊸ = ⊥-elim ([]disj∷ Δ₀ r)
cong-ccut2 [] {f = f} refl (⊸ruf {f = f₁}) = ~ (scut⊸r f f₁)
cong-ccut2 (x ∷ Δ₀) refl ⊸ruf = ⊸ruf
cong-ccut2 Δ₀ refl (⊸r p) = ⊸r (cong-ccut2 Δ₀ refl p)
cong-ccut2 Δ₀ refl ⊸rIl = ⊸rIl
cong-ccut2 Δ₀ {Δ'} r (⊸r⊸l {Γ} {Δ}) with cases++ Δ₀ Γ Δ' Δ r
cong-ccut2 {just (_ ⊸ _)} Δ₀ {_} {A} {C ⊸ _} refl (⊸r⊸l {_} {Δ}) | inj₁ (Γ₀ , refl , refl)
  rewrite cases++-inj₁ Δ₀ Γ₀ (Δ ∷ʳ C) A = ⊸r⊸l
cong-ccut2 {just (_ ⊸ _)} {Γ} .(Γ₁ ++ Γ₀) {Δ'} {A} {C ⊸ _} refl (⊸r⊸l {Γ₁} {_}) | inj₂ (Γ₀ , refl , refl) 
  rewrite cases++-inj₂ Γ₀ Γ₁ (Δ' ∷ʳ C) A = ⊸r⊸l {Δ = Γ₀ ++ Γ ++ Δ'}
cong-ccut2 {Γ = Γ} Δ₀ {Δ'} {A} r (ex⊸l₁ {Γ₁ = Γ₁} {Γ₂} {Δ} {A₁} {B}) with cases++2 Δ₀ Γ₁ Δ' (Γ₂ ++ Δ) r
cong-ccut2 {Γ = Γ} Δ₀ {_}{A} refl (ex⊸l₁ {Γ₁ = Γ₁} {Γ₂} {Δ} {A₁} {B} {B₁}) | inj₁ (Γ₀ , refl , refl) 
  rewrite cases++-inj₁ Δ₀ (Γ₀ ++ A₁ ∷ B₁ ∷ Γ₂) Δ A |
          cases++-inj₁ Δ₀ (Γ₀ ++ B₁ ∷ A₁ ∷ Γ₂) Δ A |
          cases++-inj₁ Δ₀ Γ₀ (B₁ ∷ A₁ ∷ Γ₂) A = ex⊸l₁ {Γ₁ = Δ₀ ++ Γ ++ Γ₀}
cong-ccut2 {Γ = Γ} Δ₀ {_}{A} refl (ex⊸l₁ {Γ₁ = Γ₁} {Γ₂} {Δ} {A₁} {B}) | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₁ Δ₀ (A₁ ∷ Γ₂) Δ A |
          cases++-inj₁ (Δ₀ ++ A₁ ∷ []) Γ₂ Δ A |
          cases++-inj₂ [] Δ₀ (A₁ ∷ Γ₂) A = ex-fma-cxt-⊸l₁ Δ₀ Γ
cong-ccut2 {Γ = Γ} _ {_}{A} refl (ex⊸l₁ {Γ₁ = Γ₁} {Γ₂} {Δ} {A₁} {B} {B₁}) | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
  rewrite cases++-inj₁ (Γ₁ ++ B₁ ∷ []) Γ₂ Δ A |
          cases++-inj₁ Γ₁ (B₁ ∷ Γ₂) Δ A |
          cases++-inj₂ (B₁ ∷ []) Γ₁ Γ₂ A = ex-cxt-fma-⊸l₁ Γ₁ Γ
... | inj₂ (inj₁ (Γ₀ , p , refl)) with cases++ Γ₀ Γ₂ Δ' Δ p
cong-ccut2 {Γ = Γ} _ {_}{A} refl (ex⊸l₁ {Γ₁ = Γ₁} {Γ₂} {Δ} {A₁} {B} {B₁}) | inj₂ (inj₁ (Γ₀ , refl , refl)) | inj₁ (Γ₀' , refl , refl)
  rewrite cases++-inj₁ (Γ₁ ++ B₁ ∷ A₁ ∷ Γ₀) Γ₀' Δ A |
          cases++-inj₁ (Γ₁ ++ A₁ ∷ B₁ ∷ Γ₀) Γ₀' Δ A |
          cases++-inj₂ (B₁ ∷ A₁ ∷ Γ₀) Γ₁ Γ₀' A = ex⊸l₁
cong-ccut2 {Γ = Γ} _ {Δ'} {A} refl (ex⊸l₁ {Γ₁ = Γ₁} {Γ₂} {Δ} {A₁} {B} {B₁}) | inj₂ (inj₁ (_ , refl , refl)) | inj₂ (Γ₀' , refl , refl)
  rewrite cases++-inj₂ Γ₀' (Γ₁ ++ B₁ ∷ A₁ ∷ Γ₂) Δ' A |
          cases++-inj₂ Γ₀' (Γ₁ ++ A₁ ∷ B₁ ∷ Γ₂) Δ' A = ex⊸l₁
cong-ccut2 {Γ = Γ} Δ₀ {Δ'} {A} r (ex⊸l₂ {Γ = Γ₁} {Δ₁} {Δ₂} {A₁} {B}) with cases++2 Δ₀ (Γ₁ ++ Δ₁) Δ' Δ₂ r
cong-ccut2 {Γ = Γ} _ {Δ'} {A} refl (ex⊸l₂ {Γ = Γ₁} {Δ₁} {Δ₂} {A₁} {B}) | inj₂ (inj₁ (Γ₀ , refl , refl)) 
  rewrite cases++-inj₂ (Δ₁ ++ B ∷ A₁ ∷ Γ₀) Γ₁ Δ' A |
          cases++-inj₂ (Δ₁ ++ A₁ ∷ B ∷ Γ₀) Γ₁ Δ' A |
          cases++-inj₂ (B ∷ A₁ ∷ Γ₀) Δ₁ Δ' A = ex⊸l₂
cong-ccut2 {Γ = Γ} _ {_}{A} refl (ex⊸l₂ {Γ = Γ₁} {Δ₁} {Δ₂} {A₁} {B}) | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₂ Δ₁ Γ₁ (A₁ ∷ Δ₂) A |
          cases++-inj₂ (Δ₁ ++ A₁ ∷ []) Γ₁ Δ₂ A |
          cases++-inj₂ [] Δ₁ (A₁ ∷ Δ₂) A = ex-fma-cxt-⊸l₂ Δ₁ Γ
cong-ccut2 {Γ = Γ} _ {Δ'} {A} refl (ex⊸l₂ {Γ = Γ₁} {Δ₁} {Δ₂} {A₁} {B}) | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
  rewrite cases++-inj₂ (Δ₁ ++ B ∷ []) Γ₁ Δ' A |
          cases++-inj₂ Δ₁ Γ₁ (B ∷ Δ') A |
          cases++-inj₂ (B ∷ []) Δ₁ Δ' A = ex-cxt-fma-⊸l₂ Δ₁ Γ
... | inj₁ (Γ₀ , p , refl) with cases++ Δ₀ Γ₁ Γ₀ Δ₁ p
cong-ccut2 {Γ = Γ} Δ₀ {_}{A} refl (ex⊸l₂ {Γ = Γ₁} {Δ₁} {Δ₂} {A₁} {B}) | inj₁ (_ , refl , refl) | inj₁ (Γ₀' , refl , refl)
  rewrite cases++-inj₁ Δ₀ Γ₀' (Δ₁ ++ B ∷ A₁ ∷ Δ₂) A |
          cases++-inj₁ Δ₀ Γ₀' (Δ₁ ++ A₁ ∷ B ∷ Δ₂) A = ex⊸l₂
cong-ccut2 {Γ = Γ} _ {_}{A} refl (ex⊸l₂ {Γ = Γ₁} {Δ₁} {Δ₂} {A₁} {B}) | inj₁ (Γ₀ , refl , refl) | inj₂ (Γ₀' , refl , refl)
  rewrite cases++-inj₂ Γ₀' Γ₁ (Γ₀ ++ B ∷ A₁ ∷ Δ₂) A |
          cases++-inj₂ Γ₀' Γ₁ (Γ₀ ++ A₁ ∷ B ∷ Δ₂) A |
          cases++-inj₁ Γ₀' Γ₀ (B ∷ A₁ ∷ Δ₂) A = ex⊸l₂ {Δ₁ = Γ₀' ++ Γ ++ Γ₀}
...
cong-ccut2 Δ₀ {Δ'} r (ex⊸r {Γ = Γ} {Δ}) with  cases++2 Δ₀ Γ Δ' Δ r
cong-ccut2 {Γ = Γ} Δ₀ {.(Γ₀ ++ B ∷ A₁ ∷ Δ)} {A} {A' ⊸ _} {_} {ex {Δ = Δ} {A₁} {B} _} {⊸r (ex _)} refl (ex⊸r {Γ = _} {Δ}) | inj₁ (Γ₀ , refl , refl)
  rewrite cases++-inj₁ Δ₀ Γ₀ (B ∷ A₁ ∷ Δ ++ A' ∷ []) A = ex⊸r {Γ = Δ₀ ++ Γ ++ Γ₀}
cong-ccut2 .(Γ ++ B ∷ A₁ ∷ Γ₀) {Δ'} {A} {A' ⊸ _} {_} {ex {Γ = Γ} {A = A₁} {B} _} {⊸r (ex _)} refl (ex⊸r {Γ = Γ} {_}) | inj₂ (inj₁ (Γ₀ , refl , refl))
  rewrite cases++-inj₂ (B ∷ A₁ ∷ Γ₀) Γ (Δ' ++ A' ∷ []) A = ex⊸r
cong-ccut2 {Γ = Γ} Δ₀ {.(A₁ ∷ Δ)} {A} {A' ⊸ _} {_} {ex {Δ = Δ} {A₁} _} {⊸r (ex _)} refl (ex⊸r {Γ = _} {Δ}) | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₂ [] Δ₀ (A₁ ∷ Δ ++ A' ∷ []) A = ex-fma-cxt-⊸r Δ₀ Γ
cong-ccut2 {Γ = Γ₁} .(Γ ++ B ∷ []) {Δ'} {A} {A' ⊸ _} {_} {ex {Γ = Γ} {B = B} _} {⊸r (ex _)} refl (ex⊸r {Γ = Γ} {_}) | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
  rewrite cases++-inj₂ (B ∷ []) Γ (Δ' ++ A' ∷ []) A = ex-cxt-fma-⊸r Γ Γ₁ 
...
-- parallel cuts commute and associativity of cut (all proved
-- simultaneously)

scut⊸r-hass : {T : Stp}{Γ₁ Γ₂ Δ : Cxt} → (Δ₀ : Cxt) → {Δ' : Cxt} → {A B A₂ C : Fma}
  → (f₁ : T ∣ Γ₁ ∷ʳ A ⊢ B)(f₂ : nothing ∣ Γ₂ ⊢ A₂)(g : just (A ⊸ B) ∣ Δ ⊢ C)
  → (r : Δ ≡ Δ₀ ++ A₂ ∷ Δ')
  → scut (⊸r f₁) (ccut Δ₀ f₂ g r) ≗ ccut (Γ₁ ++ Δ₀) f₂ (scut (⊸r f₁) g) (cong (_++_ Γ₁) r)
...
scutscut⊸r-vass {Γ = Γ} f (ex {Γ = Γ₁} g) h = ex {Γ = Γ ++ Γ₁} (scutscut⊸r-vass f g h)
scutscut⊸r-vass f (⊸r g) ax = refl
scutscut⊸r-vass {Γ = Γ} {Δ} f (⊸r g) (ex {Γ = Γ₁} h) = ex {Γ = Γ ++ Δ ++ Γ₁} (scutscut⊸r-vass f (⊸r g) h)
scutscut⊸r-vass f (⊸r g) (⊸r h) = ⊸r (scutscut⊸r-vass f (⊸r g) h)
scutscut⊸r-vass {Γ = Γ} {Δ = Δ} f (⊸r g) (⊸l h1 h2) =
  ~ cong-scut2 (⊸r f) (ccutscut-vass Δ h1 g h2 refl) ∙ 
  scut⊸r-hass Δ f h1 (scut g h2) refl ∙ 
  cong-ccut2 (Γ ++ Δ) refl (scutscut⊸r-vass f g h2) ∙ 
  ccutscut-vass (Γ ++ Δ) h1 (scut (⊸r f) g) h2 refl
scutscut⊸r-vass {Γ = Γ} f (⊸l g1 g2) h = scutscut-vass (ccut Γ g1 f refl) g2 h
...
scutscut-vass (⊸l f1 f2) g h = ⊸l refl (scutscut-vass f2 g h)
scutscut-vass (ex f) g h = ex (scutscut-vass f g h)

ccutscut-vass Δ₀ f ax h r = ⊥-elim ([]disj∷ Δ₀ r)
ccutscut-vass [] {_} {Δ''} f (uf g) h refl = scutscut-vass f g h
ccutscut-vass (x ∷ Δ₀) {_} {Δ''} f (uf g) h refl = uf (ccutscut-vass Δ₀ f g h refl)
ccutscut-vass Δ₀ f Ir h r = ⊥-elim ([]disj∷ Δ₀ r)
ccutscut-vass Δ₀ {Δ'} f (⊸l {Γ = Γ}{Δ} g g') h r with cases++ Δ₀ Γ Δ' Δ r
ccutscut-vass {just (_ ⊸ _)} Δ₀ {.(Γ₀ ++ Δ)} {Δ''} {A = A} f (⊸l {Γ = .(Δ₀ ++ A ∷ Γ₀)} {Δ} g g') h refl | inj₁ (Γ₀ , refl , refl)
  rewrite cases++-inj₁ Δ₀ Γ₀ (Δ ++ Δ'') A = refl
ccutscut-vass {just (_ ⊸ _)} .(Γ ++ Γ₀) {Δ'} {Δ''} {A} f (⊸l {Γ = Γ} {.(Γ₀ ++ A ∷ Δ')} g g') h refl | inj₂ (Γ₀ , refl , refl)
  rewrite cases++-inj₂ Γ₀ Γ (Δ' ++ Δ'') A = ⊸l refl (ccutscut-vass Γ₀ f g' h refl)
...
ccutscut-vass {Γ = Γ₁} .(Γ ++ B ∷ []) {Δ'} {Δ''} {A} f (ex {Γ = Γ} {Δ'} {B = B} g) h refl | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
  ...
  ∙ ~ (≡-to-≗ (scut-ex-cxt-fma Γ₁ _ _))

ccutscut⊸r-vass Δ₀ f g ax = refl
ccutscut⊸r-vass {Γ = Γ₁} Δ₀ {Δ'} {A = A} f g (ex {Γ = Γ} {Δ} {A₁} {B} h)
  rewrite cases++-inj₁ Δ₀ (Δ' ++ Γ) (B ∷ A₁ ∷ Δ) A = ex {Γ = Δ₀ ++ Γ₁ ++ Δ' ++ Γ} (ccutscut⊸r-vass Δ₀ f g h)
ccutscut⊸r-vass Δ₀ {Δ'} f g (⊸r h) = ⊸r (ccutscut⊸r-vass Δ₀ f g h)
ccutscut⊸r-vass {Γ = Γ} Δ₀ {Δ'} f g (⊸l h1 h2) =
  ccutscut-vass Δ₀ f (ccut (Δ₀ ++ _ ∷ Δ') h1 g refl) h2 refl
  ∙ cong-scut1 (ccut-hass Δ₀ f h1 g refl)
...
ccutccut-vass Γ₀ Δ₀ {Δ'} f g (⊸l {Γ = Γ}{Δ} h h') r with cases++ Δ₀ Γ Δ' Δ r
ccutccut-vass {Γ = Γ}  Γ₀ Δ₀ {_}{Γ'}{A} f g (⊸l {Δ = Δ} h h') r | inj₁ (Λ₀ , refl , refl)
  rewrite cases++-inj₁ (Δ₀ ++ Γ₀) (Γ' ++ Λ₀) Δ A = ⊸l {Γ = Δ₀ ++ Γ₀ ++ Γ ++ Γ' ++ Λ₀}{Δ} (ccutccut-vass Γ₀ Δ₀ f g h refl) refl
ccutccut-vass Γ₀ _ {Δ'}{Γ'}{A} f g (⊸l {Γ = Γ} h h') r | inj₂ (Λ₀ , refl , refl)
  rewrite cases++-inj₂ (Λ₀ ++ Γ₀) Γ (Γ' ++ Δ') A = ⊸l refl (ccutccut-vass Γ₀ Λ₀ f g h' refl)
ccutccut-vass Γ₀ Δ₀ f g (Il h) refl = Il (ccutccut-vass Γ₀ Δ₀ f g h refl)
ccutccut-vass Γ₀ Δ₀ f g (⊸r h) refl = ⊸r (ccutccut-vass Γ₀ Δ₀ f g h refl)
...
scut⊸r-hass {Γ₁ = Γ} {Δ} Δ₀ {Δ'} {A₂ = A₂} f₁ f₂ (ex {Γ = Γ₁} {Δ₁} {A} {B} g) eq with cases++2 Δ₀ Γ₁ Δ' Δ₁ eq
scut⊸r-hass {Γ₁ = Γ} {Δ} Δ₀ {A₂ = A₂} f₁ f₂ (ex {Γ = _} {Δ₁} {A} {B} g) refl | inj₁ (Γ₀ , refl , refl)
  rewrite cases++-inj₁ (Γ ++ Δ₀) Γ₀ (B ∷ A ∷ Δ₁) A₂ =
  (ex {Γ = Γ ++ Δ₀ ++ Δ ++ Γ₀}) (scut⊸r-hass Δ₀ f₁ f₂ g refl)
scut⊸r-hass {Γ₁ = Γ} {Δ} ._ {Δ'} {A₂ = A₂} f₁ f₂ (ex {Γ = Γ₁} {.(Γ₀ ++ _ ∷ Δ')} {A} {B} g) refl | inj₂ (inj₁ (Γ₀ , refl , refl)) 
  rewrite cases++-inj₂ (B ∷ A ∷ Γ₀) (Γ ++ Γ₁) Δ' A₂ =
  (ex {Γ = Γ ++ Γ₁}) (scut⊸r-hass (Γ₁ ++ _ ∷ _ ∷ Γ₀) f₁ f₂ g refl)
scut⊸r-hass {Γ₁ = Γ} {Δ} {Γ₂} Δ₀ {A₂ = A₂} f₁ f₂ (ex {Γ = Δ₀} {Δ₁} {A} g) refl | inj₂ (inj₂ (inj₁ (refl , refl , refl))) 
  rewrite cases++-inj₂ [] (Γ ++ Δ₀) (A ∷ Δ₁) A₂ =
  (≡-to-≗ (scut⊸r-ex-fma-cxt Δ f₁ (ccut (Δ₀ ++ _ ∷ []) f₂ g refl))) ∙ 
        (cong-ex-fma-cxt Δ (scut⊸r-hass (Δ₀ ++ _ ∷ []) f₁ f₂ g refl))
scut⊸r-hass {Γ₁ = Γ} {Δ} {Γ₂} _ {Δ'} {A₂ = A₂} f₁ f₂ (ex {Γ = Γ₁} {Δ'} {B = B} g) refl | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
  rewrite cases++-inj₂ (B ∷ []) (Γ ++ Γ₁) Δ' A₂ = 
  ≡-to-≗ (scut⊸r-ex-cxt-fma Δ f₁ (ccut Γ₁ f₂ g refl)) ∙ 
        cong-ex-cxt-fma {Γ = Γ ++ Γ₁} Δ (scut⊸r-hass Γ₁ f₁ f₂ g refl)
scut⊸r-hass Δ₀ f₁ f₂ (⊸r g) refl = ⊸r (scut⊸r-hass Δ₀ f₁ f₂ g refl)
scut⊸r-hass Δ₀ {Δ'} f₁ f₂ (⊸l {Γ = Γ} {Δ} g g₁) eq with cases++ Δ₀ Γ Δ' Δ eq
scut⊸r-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {.(Γ₀ ++ Δ)} {A₂ = A₂} f₁ f₂ (⊸l {Γ = .(Δ₀ ++ A₂ ∷ Γ₀)} {Δ} g g₁) refl | inj₁ (Γ₀ , refl , refl)
  rewrite cases++-inj₁ (Γ₁ ++ Δ₀) Γ₀ Δ A₂ =
  ~ (ccutscut-vass Γ₁ (ccut Δ₀ f₂ g refl) f₁ g₁ refl) ∙ 
  ~ (ccutccut-vass Δ₀ Γ₁ f₂ g (scut f₁ g₁) refl) ∙ 
  cong-ccut2 (Γ₁ ++ Δ₀) refl (ccutscut-vass Γ₁ g f₁ g₁ refl)
scut⊸r-hass {Γ₁ = Γ₁} .(Γ ++ Γ₀) {Δ'} {A₂ = A₂} f₁ f₂ (⊸l {Γ = Γ} {.(Γ₀ ++ A₂ ∷ Δ')} g g₁) refl | inj₂ (Γ₀ , refl , refl)
  rewrite cases++-inj₂ Γ₀ (Γ₁ ++ Γ) Δ' A₂ = scut-hass Γ₀ (ccut Γ₁ g f₁ refl) f₂ g₁ refl
...
scut-hass Δ₀ {Δ'} {A₂ = A₂} (⊸l {Γ} {Δ} f₁ f₃) f₂ g refl
  rewrite cases++-inj₂ (Δ ++ Δ₀) Γ Δ' A₂ = ⊸l refl (scut-hass Δ₀ f₃ f₂ g refl)
...
scut-hass Δ₀ (⊸r f₁) f₂ g refl = scut⊸r-hass Δ₀ f₁ f₂ g refl
...
ccut-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {Δ₁ = .(A ∷ Γ₀)} {Δ₂} {A₁} {A₂} f₁ f₂ (ex {Γ = .Δ₀} {.(Γ₀ ++ A₂ ∷ Δ₂)} {A} {.A₁} g) refl | inj₂ (inj₁ (Γ₀ , refl , refl)) | inj₂ (inj₂ (inj₁ (refl , refl , refl)))
  rewrite cases++-inj₂ [] Δ₀ (A ∷ Γ₀ ++ A₂ ∷ Δ₂) A₁ |
          ...
          (cong-ex-fma-cxt Γ₁ (ccut-hass (Δ₀ ++ _ ∷ []) f₁ f₂ g refl)) ∙ 
                (≡-to-≗ (sym (ccut-ex-fma-cxt2' Δ₀ Γ₁ f₂ (ccut (Δ₀ ++ A ∷ []) f₁ g refl))))
ccut-hass {Γ₁ = Γ₁} {Γ₂} .(Γ ++ B ∷ []) {Δ₁ = .Γ₀} {Δ₂} {A₁} {A₂} f₁ f₂ (ex {Γ = Γ} {.(Γ₀ ++ A₂ ∷ Δ₂)} {.A₁} {B} g) refl | inj₂ (inj₁ (Γ₀ , refl , refl)) | inj₂ (inj₂ (inj₂ (refl , refl , refl)))
  rewrite cases++-inj₂ (B ∷ []) Γ (Γ₀ ++ A₂ ∷ Δ₂) A₁ |
          ...
          (cong-ex-cxt-fma Γ₁ (ccut-hass Γ f₁ f₂ g refl)) ∙ 
                (≡-to-≗ (sym (ccut-ex-cxt-fma2' Γ Γ₁ f₂ (ccut Γ f₁ g refl))))
...
ccut-hass {Γ₂ = Γ₂} Δ₀ {_}{Δ₁}{Δ₂}{A₁}{A₂} f₁ f₂ (⊸l {Γ = Γ}{Δ} g g') r with cases++ (Δ₀ ++ A₁ ∷ Δ₁) Γ Δ₂ Δ r
ccut-hass {Γ₁ = Γ₁} {Γ₂ = Γ₂} Δ₀ {_}{Δ₁}{Δ₂}{A₁}{A₂} f₁ f₂ (⊸l {Γ = Γ}{Δ} g g') refl | inj₁ (Γ₀ , refl , refl)
  rewrite cases++-inj₁ Δ₀ (Δ₁ ++ Γ₂ ++ Γ₀) Δ A₁ |
          cases++-inj₁ Δ₀ (Δ₁ ++ A₂ ∷ Γ₀) Δ A₁ |
          cases++-inj₁ (Δ₀ ++ Γ₁ ++ Δ₁) Γ₀ Δ A₂ = ⊸l (ccut-hass Δ₀ f₁ f₂ g refl) refl
... | inj₂ (Γ₀ , refl , q) with cases++ Δ₀ Γ Δ₁ Γ₀ (sym q)
ccut-hass {Γ₁ = Γ₁} {Γ₂} Δ₀ {_}{_} {Δ₂} {A₁} {A₂} f₁ f₂ (⊸l g g') refl | inj₂ (Γ₀ , refl , refl) | inj₁ (Γ₀' , refl , refl)
  rewrite cases++-inj₁ Δ₀ Γ₀' (Γ₀ ++ A₂ ∷ Δ₂) A₁ |
          cases++-inj₁ Δ₀ Γ₀' (Γ₀ ++ Γ₂ ++ Δ₂) A₁ |
          cases++-inj₂ Γ₀ (Δ₀ ++ Γ₁ ++ Γ₀') Δ₂ A₂ = refl
ccut-hass {Γ₁ = Γ₁} {Γ₂} ._ {_} {Δ₁} {Δ₂} {A₁} {A₂} f₁ f₂ (⊸l {Γ = Γ} g g') refl | inj₂ (._ , refl , refl) | inj₂ (Γ₀ , refl , refl)
  rewrite cases++-inj₂ Γ₀ Γ (Δ₁ ++ A₂ ∷ Δ₂) A₁ |
          cases++-inj₂ Γ₀ Γ (Δ₁ ++ Γ₂ ++ Δ₂) A₁ |
          cases++-inj₂ (Γ₀ ++ Γ₁ ++ Δ₁) Γ Δ₂ A₂ = ⊸l refl (ccut-hass Γ₀ f₁ f₂ g' refl)
ccut-hass Δ₀ f₁ f₂ (Il g) refl = Il (ccut-hass Δ₀ f₁ f₂ g refl)
ccut-hass Δ₀ f₁ f₂ (⊸r g) refl = ⊸r (ccut-hass Δ₀ f₁ f₂ g refl)
